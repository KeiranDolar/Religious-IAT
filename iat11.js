...
//define(['pipAPI','/implicit/user/yba/testNewScorer/nogaScorer.js','/implicit/user/yba/testNewScorer/feedbacker.js','underscore'], function(APIConstructor, Scorer, Feedbacker, _) {
	...
	**/
	...
	function iatExtension(options)
	{
        ...
        //YBYB: Our attempts to use a class for the scorer failed, so this is a workaround.
        global.scorer = {};
		scorer_init(global.scorer);
        global.feedbacker = {};
		feedbacker_init(global.feedbacker);
		...
		var iatObj =
		{
			...
			//D Scoring parameters
			// All recommended scoring: fastRT = 300, maxFastTrialsRate : 0.1, maxErrorRate : 1 [unlike the default], maxRT = 10000
			// D1 = errorScoring : 'latency', minRT : 0  
			// D2 = errorScoring : 'latency'; minRT : 400 
			// D4 = errorScoring : 'penalty', minRT : 0  
			// D6 = errorScoring : 'penalty'; minRT : 400 
			// D3 and D5 are not supported (the penalty is 2SD)
			fastRT : 300, //Below this latency = extremely fast (used for maxFastTrialsRate).
			maxFastTrialsRate : 0.1, //Above this % of extremely fast responses within a condition, the participant is considered too fast.
			maxErrorRate : 1, // The maximum error-rate in the critical blocks.
		...
		*/
		...
		API.addSettings('hooks',{
				endTask: function(){
				    global.scorer.logs = global.current.logs; 
				    // Convert the object to a JSON string const 
				    global.scoreObjStr = JSON.stringify(global.scorer, null, 2); 
				    // Log the JSON string to the console 
				    //console.log(global.scoreObjStr);
				    var DScoreObj = global.scorer.computeD(global.scorer);
					//console.log("DScoreObj:", DScoreObj);
					global.feedbacker.DScoreObj = DScoreObj;
					piCurrent.feedback = global.feedbacker.getFeedback(global.feedbacker);
		...
		/**
		*Settings and parameters for the scorer and the feedbacker
		**/
		
		var errorLatencyUse = piCurrent.errorCorrection ? piCurrent.errorScoring : 'penalty';
		//console.log('errorLatencyUse='+errorLatencyUse + ' piCurrent.errorCorrection='+piCurrent.errorCorrection);
		//Settings for the score computation.
		global.scorer.addSettings(global.scorer, {
			errorVar:'score', 
			condVar:'condition',
			//condition 1
			cond1VarValues: [
			...
			parcelVar : "parcel", //Which variable in the data indicates the parcel.
			parcelValues : ['first', 'second'],
			fastRT : piCurrent.fastRT, //Below this latency = extremely fast (used for maxFastTrialsRate).
			maxFastTrialsRate : piCurrent.maxFastTrialsRate, //Above this % of extremely fast responses within a condition, the participant is considered too fast.
			maxErrorRate : piCurrent.maxErrorRate, //The maximum error-rate in the critical blocks.
			minRT : piCurrent.minRT, //Delete trials below this latency
			maxRT : piCurrent.maxRT, //Delete trials above this
			errorLatency : {use:errorLatencyUse, penalty:600, useForSTD:true}
		});

        //// Set the settings for the object that determines the participant's feedback
		...
		var messageDef = [
				{ cut:'-0.65', message : getFB(piCurrent.fb_strong_Att1WithCatA_Att2WithCatB, cat2Name, cat1Name, att1Name, att2Name) },
				{ cut:'-0.35', message : getFB(piCurrent.fb_moderate_Att1WithCatA_Att2WithCatB, cat2Name, cat1Name, att1Name, att2Name) },
				{ cut:'-0.15', message : getFB(piCurrent.fb_slight_Att1WithCatA_Att2WithCatB, cat2Name, cat1Name, att1Name, att2Name) },
				{ cut:'0.15', message : getFB(piCurrent.fb_equal_CatAvsCatB, cat1Name, cat2Name, att1Name, att2Name) },
				{ cut:'0.35', message : getFB(piCurrent.fb_slight_Att1WithCatA_Att2WithCatB, cat1Name, cat2Name, att1Name, att2Name) },
				{ cut:'0.65', message : getFB(piCurrent.fb_moderate_Att1WithCatA_Att2WithCatB, cat1Name, cat2Name, att1Name, att2Name) },
				{ cut:'5', message : getFB(piCurrent.fb_strong_Att1WithCatA_Att2WithCatB, cat1Name, cat2Name, att1Name, att2Name) }
		];


		var feedbackerParams = { messageDef : messageDef };
		if (piCurrent.manyErrors !== '')
		{
			feedbackerParams.manyErrors = piCurrent.manyErrors;
		}
		if (piCurrent.tooFast !== '')
		{
			feedbackerParams.tooFast = piCurrent.tooFast;
		}
		if (piCurrent.notEnough !== '')
		{
			feedbackerParams.notEnough = piCurrent.notEnough;
		}
		feedbackerParams.maxFastTrialsRate = piCurrent.maxFastTrialsRateFB; //Above this % of extremely fast responses within a condition, the participant is considered too fast for feedback.
		feedbackerParams.maxErrorRate = piCurrent.maxErrorRateFB; //Above this % of extremely fast responses within a condition, the participant is considered too fast for feedback.
		feedbackerParams.minTotalScoredTrials = piCurrent.minTotalScoredTrialsFB;
		
		//Set parameters to the feedback object.
		global.feedbacker.addSettings(global.feedbacker, feedbackerParams);
